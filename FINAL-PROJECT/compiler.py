...
def compile_stmt(stmt):
    ...
    if stype == "QubitDecl":
        ...
        return {"type": "if","condition": stmt["condition"],"then": [compile_stmt(s) for s in then_stmts],
                "else": [compile_stmt(s) for s in else_stmts]}
    else:
        raise ValueError(f"Unknown statement type: {stype}")
def ast_to_ir(ast):
    # Wrap single statement AST (like {"type": "Convert", ...}) into a full Program
